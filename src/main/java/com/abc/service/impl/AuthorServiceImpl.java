...
public class AuthorServiceImpl implements AuthorService {
	...
	public ResponseEntity<List<AuthorResponse>> saveAll(List<Author> authors) {
		authors.forEach(author -> author.setPassword(bCryptPasswordEncoder.encode(author.getPassword())));
		List<AuthorResponse> responses = new ArrayList<>();
		this.authorRepository.saveAll(authors).forEach(author -> {
			AuthorResponse reponse = new AuthorResponse();
			BeanUtils.copyProperties(author, reponse);
